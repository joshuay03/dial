...
module Dial
  class Panel
    ...
    class << self
      def html env, headers, profile_uuid, query_logs, ruby_vm_stat, gc_stat, gc_stat_heap, server_timing
        <<~HTML
          <style>#{style}</style>
          ...
          <div id="dial">
            <div id="dial-preview">
              <span id="dial-preview-header">
                ...
                #{formatted_profile_output env, profile_uuid}
              </span>
      ...
      def formatted_profile_output env, profile_uuid
        url_base = ::Rails.application.routes.url_helpers.dial_url host: env[::Rack::HTTP_HOST]
        prefix = "/" unless url_base.end_with? "/"
        profile_out_url = URI.encode_www_form_component url_base + "#{prefix}dial/profile?uuid=#{profile_uuid}"
